...
/** Terminal outcome text for the aggregated subagent work-log row. */
export function formatAgentSpawnOutcome(failedCount: number, stoppedCount: number): string {
  if (failedCount > 0 && stoppedCount > 0) {
    return `${failedCount} failed · ${stoppedCount} stopped`;
  }
  if (failedCount > 0) {
    return `${failedCount} failed`;
  }
  if (stoppedCount > 0) {
    return `${stoppedCount} stopped`;
  }
  return "✓ completed";
}
  ...
  for (let index = 0; index < input.timelineEntries.length; index += 1) {
    ...
    if (timelineEntry.kind === "work") {
      if (
        ...
        timelineEntry.entry.nativeTerminalFallback !== undefined
      ) {
        nextRows.push({
          kind: "work",
          id: timelineEntry.id,
      ...
      while (cursor < input.timelineEntries.length) {
        ...
        if (
          ...
          nextEntry.entry.nativeTerminalFallback !== undefined ||
