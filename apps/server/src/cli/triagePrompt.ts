 ...
 *
 * `TRIAGE_PLAYBOOK` is the offline copy embedded in installed builds. When reachable,
 * triage fetches the current playbook from the KM Code fork's `main` branch.
 */

export const TRIAGE_PLAYBOOK = `# KM Code triage playbook

You are a support engineer for KM Code (a fork of T3 Code), working
...
Fetch https://raw.githubusercontent.com/kmccleary3301/t3code/main/.github/triage/PLAYBOOK.md.
If it is reachable and its content differs from this text, follow that version only
when it identifies KM Code and the kmccleary3301/t3code fork; otherwise keep this
copy. The user may be on an old release with an old copy.

## 4. Get the source

Before cloning, check the release tag/build hint in the context file against the actual
tags in https://github.com/kmccleary3301/t3code. Do not invent a v-prefixed version tag:
use a verified KM Code fork tag when one exists, or inspect the local build metadata
and check out the matching commit/source when the tag is unavailable.

Clone the repo at the verified tag (or matching commit), into the source cache directory
named in the context file, one subdirectory per commit hash:

    git clone --depth 1 --filter=blob:none --branch <verified-release-tag> \\
      https://github.com/kmccleary3301/t3code <source-cache-dir>/<hash>

If no matching tag or commit can be identified (for example, a local build), clone
\`main\` instead, and treat file and line references as approximate: the user's build
may not match \`main\` exactly. If the target directory already exists from an earlier
triage run, reuse it instead of cloning again. Before cloning, delete other entries
in the source cache directory, but only entries whose git state is clean (no
...
- How is KM Code running on this machine: \`npx t3 serve\` in a terminal, the
...
## 6. Check the current KM Code release

Search existing issues in kmccleary3301/t3code (use \`gh\`, or the public GitHub search
...
Do not patch the KM Code source as a fix. A good issue with strong repro steps
...
of \`main\` from the KM Code fork for that work, never the tag-pinned diagnosis clone.
...
- If \`gh\` is not authenticated, offer \`gh auth login\`, or build a prefilled
  https://github.com/kmccleary3301/t3code/issues/new URL with title and body query
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your KM Code triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A KM Code user is \
...
export const buildTriageContext = (input: TriageContextInput) => `# KM Code triage context

Generated by \`t3 triage\` at ${input.generatedAt}.

- Installed version: ${input.version}
- Release tag candidate (verify before checkout): ${input.releaseTag}
- OS: ${input.os}
- Node: ${input.nodeVersion}
- CLI launched as: ${input.launchedAs}
- Server process: ${input.server}
- Repo: https://github.com/kmccleary3301/t3code
