...
it("limits mentions to the configured subscription role despite release text", () => {
  const payload = buildDiscordReleaseAnnouncement({
    ...latestAnnouncement,
    releaseName: "Release @everyone <@123456789>",
  });
  assert.deepStrictEqual(payload.allowed_mentions, { roles: [latestAnnouncement.roleId] });
});
